...
def getCredibleInterval(Hess, K):

    #Hess_dense = Hess.toarray()


    # Optional: regularize to stabilize inversion
    #regularize = 1e-3
    #Hess_dense += np.eye(Hess_dense.shape[0]) * regularize
    inv_diag = invDiagHess(Hess, K)
    #inv_diag = np.maximum(inv_diag, 0)
    return np.sqrt(inv_diag).reshape(K,-1)


def invDiagHess(Hess, K):
    '''
